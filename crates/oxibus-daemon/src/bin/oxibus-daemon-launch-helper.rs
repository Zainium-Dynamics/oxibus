...
use oxibus_daemon::launch_helper::{
    HelperError, TRUSTED_CONFIG_PATH, check_permissions, find_service, parse_args, service_dirs,
};
...
fn run(bus_name: &str) -> Result<(), HelperError> {
    ...
    for path in &paths_to_try {
        if std::path::Path::new(path).exists() {
            match oxibus_config::GlobalConfig::load_path(path) {
                ...
                Err(e) => {
                    last_err = Some(HelperError::ConfigInvalid(
                        (*path).to_string(),
                        e.to_string(),
                    ));
                }
    ...
    let config = match loaded_config {
        ...
        None => {
            return Err(last_err.unwrap_or_else(|| {
                HelperError::ConfigInvalid(
                    TRUSTED_CONFIG_PATH.to_string(),
    ...
    Err(HelperError::ExecFailed(format!(
        "failed to exec '{}': {err}",
        def.exec
    )))
}
...
fn clear_environment() {
    ...
    unsafe {
        ...
        std::env::set_var(
            "PATH",
            "/overlayer/syshub/bin:/overlayer/syshub/sbin:/usr/bin:/usr/sbin:/bin:/sbin",
        );
...
fn switch_user(user: &str) -> Result<(), HelperError> {
    ...
    unsafe {
        ...
        if libc::initgroups(cname.as_ptr(), gid) != 0 {
            return Err(HelperError::SetupFailed(
                "could not initialize groups".into(),
            ));
        }
